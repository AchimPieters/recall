...
Use `docker/docker-compose.yml` for local stacks with API + worker + agent + frontend + PostgreSQL + Redis.
...
## Agent defaults
- `agent/agent.py` defaults to `https://localhost:8000` and TLS verification enabled.
