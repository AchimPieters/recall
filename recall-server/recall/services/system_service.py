...
class SystemService:
    ...
    def _audit(self, action: str, message: str) -> None:
        self.db.add(DeviceLog(device_id="system", level="audit", action=action, message=message))
        self.db.commit()
    ...
    def reboot(self, confirmed: bool, actor: str) -> dict:
        ...
        proc = subprocess.run(["systemctl", "reboot"], capture_output=True, text=True)
    ...
    def update(self, confirmed: bool, actor: str) -> dict:
        ...
        proc = subprocess.run(["bash", "/opt/recall/update.sh"], capture_output=True, text=True)
        return {"ok": proc.returncode == 0, "stdout": proc.stdout.strip(), "stderr": proc.stderr.strip()}
