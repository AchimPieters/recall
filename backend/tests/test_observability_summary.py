...
def test_observability_summary_is_tenant_scoped_for_org_user(monkeypatch) -> None:
    ...
    other_org_id = 7008
    _ensure_user(
        "obs-operator", "OperatorPass1!", role="operator", organization_id=tenant_org_id
    )
    monkeypatch.setattr(
        celery_module.celery_app.control, "inspect", lambda timeout=1.0: _InspectOK()
    )
    ...
    try:
        svc = DeviceService(db)
        before_tenant_device_count = (
            db.query(Device).filter(Device.organization_id == tenant_org_id).count()
        )
        before_tenant_alert_count = (
            db.query(Alert).filter(Alert.organization_id == tenant_org_id).count()
        )

        svc.register(
            "obs-org-1-7007", "Org Device", None, "1.0.0", organization_id=tenant_org_id
        )
        svc.register(
            "obs-org-2-7008",
            "Other Org Device",
