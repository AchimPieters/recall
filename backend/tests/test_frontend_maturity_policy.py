...
def test_frontend_tooling_policy_scripts_and_configs_present() -> None:
    ...
    missing_scripts = [
        name for name, token in required_scripts.items() if name not in scripts or token not in scripts[name]
    ]
    assert not missing_scripts, "Frontend maturity scripts missing: " + ", ".join(missing_scripts)
    ...
    assert not missing_deps, "Frontend maturity dev dependencies missing: " + ", ".join(missing_deps)
