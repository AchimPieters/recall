...
def test_media_duplicate_detection_and_versioning(tmp_path: Path, monkeypatch) -> None:
    ...
    svc = MediaService(db)
    first = svc.store_upload(
        "banner.png", "image/png", _png_bytes("red"), organization_id=1
    )
    same = svc.store_upload(
        "copy.png", "image/png", _png_bytes("red"), organization_id=1
    )
    assert same.id == first.id

    updated = svc.store_upload(
        "banner.png", "image/png", _png_bytes("blue"), organization_id=1
    )
...
def test_image_metadata_is_stored(tmp_path: Path, monkeypatch) -> None:
    ...
    svc = MediaService(db)
    media = svc.store_upload(
        "poster.png", "image/png", _png_bytes("green", (20, 30)), organization_id=1
    )
...
def test_corrupt_image_rejected(tmp_path: Path, monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match="Corrupt image upload"):
        svc.store_upload(
            "broken.png", "image/png", b"not-a-real-image", organization_id=1
        )
...
def test_validate_upload_rejects_invalid_video_container(
    tmp_path: Path, monkeypatch
) -> None:
    db = _db_session()
    monkeypatch.setattr(media_service_module.settings, "media_dir", str(tmp_path))
...
def test_media_workflow_default_and_valid_transitions(
    tmp_path: Path, monkeypatch
) -> None:
    db = _db_session()
    monkeypatch.setattr(media_service_module.settings, "media_dir", str(tmp_path))
    ...
    media = svc.store_upload(
        "poster.png", "image/png", _png_bytes("green"), organization_id=1
    )
...
def test_media_workflow_rejects_invalid_or_forbidden_transitions(
    tmp_path: Path, monkeypatch
) -> None:
    db = _db_session()
    monkeypatch.setattr(media_service_module.settings, "media_dir", str(tmp_path))
...
def test_media_workflow_transition_enforces_organization_scope(
    tmp_path: Path, monkeypatch
) -> None:
    db = _db_session()
    monkeypatch.setattr(media_service_module.settings, "media_dir", str(tmp_path))
